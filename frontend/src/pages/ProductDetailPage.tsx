          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="space-y-3">
              {product.images?.map((img, index) => (
                <button type="button" key={img.url} className={`rounded-lg border p-2 ${index === 0 ? 'border-primary' : 'border-border hover:border-primary'}`}>
          ...
          <div className="max-w-4xl">
            ...
            <div className="flex items-center gap-2 mb-4">
              {Array.from({ length: 5 }).map((_, index) => (
                <Star key={index} className={`h-4 w-4 ${index < Math.round(product.rating) ? 'fill-current text-yellow-500' : 'text-muted'}`} />
              ))}
              <span className="text-muted ml-2">{product.rating} ({product.stock} in stock)</span>
            </div>
            <div className="flex items-baseline gap-2 mb-6">
              <span className="text-2xl font-bold text-primary">${product.price.toFixed(2)}</span>
              {product.originalPrice && product.originalPrice > product.price && <s className="text-sm text-muted">${product.originalPrice.toFixed(2)}</s>}
              {product.discount > 0 && <span className="text-sm text-success">{product.discount}% off</span>}
            </div>
            <p className="text-foreground leading-7 mb-6">{product.description}</p>
            <Separator className="mb-6" />
            <div className="flex items-center gap-3 mb-6">
              <Truck className="h-5 w-5 text-primary" />
              <div><p className="font-medium">Fast and reliable delivery</p><p className="text-sm text-muted">Usually delivered within 3–5 business days.</p></div>
            </div>
            <button type="button" disabled={product.stock <= 0} className="rounded-md bg-primary px-6 py-3 text-primary-foreground font-medium disabled:cursor-not-allowed disabled:opacity-50">
              {product.stock > 0 ? 'Add to Cart' : 'Out of Stock'}
            </button>
            <div className="mt-6 text-sm text-muted">SKU: {product.sku}</div>
          </div>
        </div>
      </main>
    </div>
