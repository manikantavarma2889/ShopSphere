...
export const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <header className="py-20 md:py-24">
        <div className="max-w-7xl mx-auto px-4">
          <section className="mb-12 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button type="button" onClick={() => window.location.href = '/products'} className="inline-flex items-center px-6 py-3 bg-primary text-primary-foreground rounded-full hover:bg-primary/90">
                Start Shopping <ArrowRight className="ml-2 h-4 w-4" />
              </button>
              <button type="button" onClick={() => setSelectedCategory('All')} className="inline-flex items-center px-6 py-3 border border-border rounded-full hover:bg-background">
                Browse Categories
              </button>
            </div>
          </section>
          ...
          <section className="mb-12">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {categories.map((category) => (
                <button type="button" key={category.name} onClick={() => setSelectedCategory(category.name)} className="text-left">
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              ...
              <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search..." className="rounded-lg border border-border px-3 py-2 text-sm" />
            </div>
          ...
          <section className="mt-12 pt-8 border-t border-border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
              <div><p className="text-3xl font-bold text-primary">10K+</p><p className="text-sm text-muted">Happy Customers</p></div>
              <div><p className="text-3xl font-bold text-primary">500+</p><p className="text-sm text-muted">Products</p></div>
              <div><p className="text-3xl font-bold text-primary">98%</p><p className="text-sm text-muted">Positive Reviews</p></div>
              <div><p className="text-3xl font-bold text-primary">24/7</p><p className="text-sm text-muted">Support</p></div>
            </div>
          </section>
        </div>
      </header>
