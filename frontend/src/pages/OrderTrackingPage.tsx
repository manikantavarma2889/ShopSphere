    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-16">
        <div className="max-w-7xl mx-auto px-4">
              ...
              {timelineSteps.map((step, index) => (
                <div key={step.label} className="flex items-start gap-4">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                    ...
                    step.status === 'current' ? 'text-primary bg-primary/10 border-2 border-primary' :
                    'text-muted bg-border/10'
                  }`}>
                  ...
                  {index < timelineSteps.length - 1 && <div className="w-px h-6 bg-border/50 mx-2" />}
                </div>
              ))}
            </div>
          ...
          <div className="mt-8">
            <button type="button" onClick={() => window.location.href = '/'} className="px-6 py-3 bg-primary text-primary-foreground rounded hover:bg-primary/90">Continue Shopping</button>
          </div>
        </div>
      </main>
    </div>
